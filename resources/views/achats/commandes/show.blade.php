...
<div class="space-y-3">
    ...
    <div class="bg-white rounded-[4px] border border-gray-300 p-5">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-3 flex-wrap">
                <h1 class="text-[15px] font-bold text-gray-900 font-mono">{{ $purchaseOrder->number }}</h1>
                @switch($purchaseOrder->status)
                    ...
                    @case('annule')
                        ...
                        @break
                @endswitch
                <span class="text-gray-500 text-[13px]">{{ $purchaseOrder->supplier?->name }}</span>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <form action="{{ route('achats.commandes.duplicate', $purchaseOrder) }}" method="POST"
                    ...
                    @csrf
                    <button type="submit"
                            class="inline-flex items-center gap-2 px-3 py-2 bg-slate-600 text-white rounded-[4px] text-[13px] font-medium hover:bg-slate-700 transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-[4px] border border-gray-300 p-5 space-y-4">
            ...
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-[13px]">
                <div>
                    ...
                    <dd class="text-gray-500 text-[12px]">{{ $purchaseOrder->supplier->city }}</dd>
                    @endif
                </div>
                <div>
                ...
                <div>
                    <dt class="text-[12px] font-medium text-gray-500 uppercase tracking-wider">Date de commande</dt>
        ...
        <div class="bg-white rounded-[4px] border border-gray-300 p-5 space-y-3 h-fit">
            ...
            <div class="border-t border-gray-200 pt-3 flex justify-between">
                ...
                <span class="text-[13px] font-bold text-amber-700 tabular-nums">{{ number_format($purchaseOrder->total_ttc, 0, ',', ' ') }} FCFA</span>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[4px] border border-gray-300 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-200">
            <h2 class="text-[13px] font-semibold text-gray-900">Lignes de commande</h2>
        </div>
    ...
    <div class="bg-white rounded-[4px] border border-gray-300 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-200 flex items-center justify-between">
            <h2 class="text-[13px] font-semibold text-gray-900">Réceptions liées</h2>
            <span class="text-[13px] text-gray-500">{{ $purchaseOrder->receptions->count() }} réception(s)</span>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full divide-y divide-gray-200 text-[13px]">
                <thead class="bg-[#eef5f0] border-b border-gray-300">
    ...
    <div class="bg-white rounded-[4px] border border-gray-300 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-200 flex items-center justify-between">
            <h2 class="text-[13px] font-semibold text-gray-900">Factures fournisseurs liées</h2>
            <span class="text-[13px] text-gray-500">{{ $purchaseOrder->supplierInvoices->count() }} facture(s)</span>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full divide-y divide-gray-200 text-[13px]">
                <thead class="bg-[#eef5f0] border-b border-gray-300">
