            ...
            <div class="p-4 grid grid-cols-1 sm:grid-cols-12 gap-x-4 gap-y-3">
                ...
                <div class="sm:col-span-3 space-y-3">
                    ...
                    <div>
                        ...
                        <span class="inline-flex items-center gap-1 h-8 text-[12px] font-semibold px-2.5 rounded-[3px] {{ ($fi?->status ?? 'brouillon') === 'brouillon' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-gray-100 text-gray-600 border border-gray-200' }}">● {{ ucfirst($fi?->status ?? 'Brouillon') }}</span>
                    </div>
                ...
                <div class="sm:col-span-3 space-y-3">
                    ...
                    <div><label class="{{ $lbl }}">Régime fiscal</label><input type="text" name="fiscal_regime" maxlength="40" value="{{ old('fiscal_regime', $fi?->fiscal_regime) }}" class="{{ $inp }}" placeholder="Ex. : Régime réel normal"></div>
                    {{-- [UI — doublon retiré] Champ « Taxes » (`default_tax_label`)
                         supprimé, comme sur devis, commande et facture de vente : libellé
                         stocké, validé, `fillable`, mais JAMAIS lu par aucune logique
                         métier ni affiché sur aucune fiche ou PDF. Il doublonnait la TVA
                         portée par les lignes et pouvait la contredire.
                         La valeur est DÉRIVÉE de l'état réel dans SupplierInvoiceService. --}}
                    <div><label class="{{ $lbl }}">Projet</label><input type="text" name="project_reference" maxlength="60" value="{{ old('project_reference', $fi?->project_reference) }}" class="{{ $inp }} font-mono" placeholder="Ex. : PROJ-2026-0008"></div>
                    <div>
