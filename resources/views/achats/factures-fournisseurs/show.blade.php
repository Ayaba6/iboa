...
<div class="space-y-3">
    ...
    <div class="bg-white rounded-[4px] border border-gray-300 p-5">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-3 flex-wrap">
                <h1 class="text-[15px] font-bold text-gray-900 font-mono">{{ $invoice->number }}</h1>
                ...
                <span class="inline-flex items-center px-2 py-0.5 rounded text-[12px] font-semibold bg-red-100 text-red-700">En retard</span>
                @endif
                {{-- [INVOICE-LOCKED-GUARD] --}}
                @if($invoice->status === 'payee' || (int) $invoice->remaining_amount === 0)
                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[12px] font-bold bg-gray-800 text-white" title="Facture entièrement réglée — aucun nouveau décaissement n'est autorisé">
                    <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clip-rule="evenodd"/></svg>
                    VERROUILLÉE
                </span>
                @endif
                <span class="text-gray-500 text-[13px]">{{ $invoice->supplier?->name }}</span>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2" x-data="{ payModal: false }">
                ...
                <button type="button" @click="payModal = true"
                        class="inline-flex items-center gap-2 px-3 py-2 bg-green-600 text-white rounded-[4px] text-[13px] font-medium hover:bg-green-700 transition-colors">
                ...
                <div x-show="payModal" x-cloak
                    ...
                    <div @click.outside="payModal = false"
                        ...
                        <h3 class="text-[15px] font-semibold text-gray-900">Enregistrer un paiement</h3>
                        ...
                        <form action="{{ route('achats.factures-fournisseurs.payment', $invoice) }}" method="POST" class="space-y-4">
                            @csrf
                            <div>
                                <label class="block text-[13px] font-medium text-gray-700 mb-1">Montant <span class="text-red-500">*</span></label>
                                <input type="number" name="amount" min="1" max="{{ $invoice->remaining_amount }}"
                                       value="{{ $invoice->remaining_amount }}" required
                                       class="w-full rounded-[4px] border border-gray-300 px-3 py-2 text-[13px] focus:ring-1 focus:ring-amber-500 focus:border-amber-500">
                            </div>
                            <div>
                                <label class="block text-[13px] font-medium text-gray-700 mb-1">Date de paiement <span class="text-red-500">*</span></label>
                                <input type="date" name="payment_date" value="{{ date('Y-m-d') }}" required
                                       class="w-full rounded-[4px] border border-gray-300 px-3 py-2 text-[13px] focus:ring-1 focus:ring-amber-500 focus:border-amber-500">
                            </div>
                            <div>
                                <label class="block text-[13px] font-medium text-gray-700 mb-1">Mode de paiement</label>
                                <select name="payment_method_id" class="w-full rounded-[4px] border border-gray-300 px-3 py-2 text-[13px] focus:ring-1 focus:ring-amber-500 focus:border-amber-500">
                            ...
                            <div>
                                <label class="block text-[13px] font-medium text-gray-700 mb-1">Compte de trésorerie</label>
                                <select name="cash_account_id" class="w-full rounded-[4px] border border-gray-300 px-3 py-2 text-[13px] focus:ring-1 focus:ring-amber-500 focus:border-amber-500">
                                    <option value="">— Sélectionner —</option>
                            ...
                            @endif
                            <div>
                                <label class="block text-[13px] font-medium text-gray-700 mb-1">Référence</label>
                                <input type="text" name="reference" placeholder="N° chèque, virement..."
                                       class="w-full rounded-[4px] border border-gray-300 px-3 py-2 text-[13px] focus:ring-1 focus:ring-amber-500 focus:border-amber-500">
                            </div>
                            <div class="flex justify-end gap-3 pt-2">
                                <button type="button" @click="payModal = false"
                                        class="px-3 py-1.5 rounded-[4px] border border-gray-300 text-[13px] text-gray-700 hover:bg-gray-50">
                                    Annuler
                                </button>
                                <button type="submit"
                                        class="px-3 py-1.5 rounded-[4px] bg-green-600 text-white text-[13px] font-medium hover:bg-green-700">
                                    Enregistrer
                                </button>
                            </div>
                        </form>
                ...
                <form action="{{ route('achats.factures-fournisseurs.validate', $invoice) }}" method="POST"
                    ...
                    @csrf
                    <button type="submit"
                            class="inline-flex items-center gap-2 px-3 py-2 bg-emerald-700 text-white rounded-[4px] text-[13px] font-medium hover:bg-emerald-800 transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <form action="{{ route('achats.factures-fournisseurs.destroy', $invoice) }}" method="POST"
                    ...
                    @csrf @method('DELETE')
                    <button type="submit"
                            class="inline-flex items-center gap-2 px-3 py-2 bg-red-600 text-white rounded-[4px] text-[13px] font-medium hover:bg-red-700 transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('achats.factures-fournisseurs.index') }}"
                   class="inline-flex items-center gap-2 px-3 py-2 border border-gray-300 text-gray-700 rounded-[4px] text-[13px] font-medium hover:bg-gray-50 transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-[4px] border border-gray-300 p-5 space-y-4">
            ...
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-[13px]">
                <div>
                    <dt class="text-[12px] font-medium text-gray-500 uppercase tracking-wider">Fournisseur</dt>
                    <dd class="mt-0.5 font-semibold text-gray-900">{{ $invoice->supplier?->name ?? '—' }}</dd>
                    @if($invoice->supplier?->city)
                    <dd class="text-gray-500 text-[12px]">{{ $invoice->supplier->city }}</dd>
                    @endif
                    @if($invoice->supplier?->phone)
                    <dd class="text-gray-500 text-[12px]">{{ $invoice->supplier->phone }}</dd>
                    @endif
                </div>
                <div>
                    <dt class="text-[12px] font-medium text-gray-500 uppercase tracking-wider">Numéro interne</dt>
                    <dd class="mt-0.5 font-mono font-semibold text-gray-900">{{ $invoice->number }}</dd>
                    @if($invoice->supplier_invoice_number)
                    <dd class="text-gray-500 text-[12px]">Réf. fourn. : {{ $invoice->supplier_invoice_number }}</dd>
                    @endif
                </div>
                <div>
                    <dt class="text-[12px] font-medium text-gray-500 uppercase tracking-wider">Date de réception</dt>
                ...
                <div>
                    ...
                    <dd class="mt-0.5 {{ $overdue ? 'text-red-600 font-semibold' : 'text-gray-700' }}">
                        ...
                        @if($overdue)
                        <span class="ml-1 text-[12px] text-red-500">(en retard)</span>
                        @endif
                    </dd>
                </div>
                @if($invoice->purchaseOrder)
                <div>
                    <dt class="text-[12px] font-medium text-gray-500 uppercase tracking-wider">Commande achat liée</dt>
                    <dd class="mt-0.5">
        ...
        <div class="bg-white rounded-[4px] border border-gray-300 p-5 space-y-3 h-fit">
            <h2 class="text-[13px] font-semibold text-gray-900">Récapitulatif</h2>
            ...
            <div class="border-t border-gray-200 pt-3 flex justify-between">
                ...
                <span class="text-[13px] font-bold text-amber-700 tabular-nums">{{ number_format($invoice->total_ttc, 0, ',', ' ') }} FCFA</span>
            </div>
    ...
    <div class="bg-white rounded-[4px] border border-gray-300 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-200">
            <h2 class="text-[13px] font-semibold text-gray-900">Lignes de la facture</h2>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full divide-y divide-gray-200 text-[13px]">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="px-4 py-16 text-center text-gray-400 text-[13px]">Aucun résultat.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="bg-white rounded-[4px] border border-gray-300 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-200 flex items-center justify-between">
            <h2 class="text-[13px] font-semibold text-gray-900">Paiements</h2>
            <span class="text-[13px] text-gray-500">{{ $invoice->payments->count() }} paiement(s)</span>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full divide-y divide-gray-200 text-[13px]">
                <thead class="bg-[#eef5f0] border-b border-gray-300">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-3 py-1.5 text-gray-600 font-mono text-[12px]">{{ $payment->reference ?? '—' }}</td>
