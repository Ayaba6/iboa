        ...
        <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <div>
                <div class="flex items-center gap-3 flex-wrap">
                    <h1 class="text-[16px] font-bold text-gray-900 font-mono">{{ $rfq->number }}</h1>
            ...
            <div class="flex flex-wrap items-center gap-2">
                @can('purchase_orders.create')
                    ...
                    <form action="{{ route('achats.rfq.send', $rfq) }}" method="POST" onsubmit="return confirm('Marquer la RFQ comme envoyée à tous les fournisseurs ?')">
                        @csrf
                        <button type="submit" class="bg-emerald-700 hover:bg-emerald-800 text-white text-sm font-medium px-3 py-1.5 rounded-[4px]">📤 Marquer envoyée</button>
                    </form>
                    @endif
                    @if(!$rfq->isClosed() && !$rfq->isCancelled())
                    <a href="{{ route('achats.rfq.compare', $rfq) }}" class="bg-emerald-700 hover:bg-emerald-800 text-white text-sm font-medium px-3 py-1.5 rounded-[4px]">📊 Comparatif</a>
                    @endif
                    @if($rfq->isDraft())
                    ...
                    @endif
                @endcan
                <a href="{{ route('achats.rfq.index') }}" class="border border-gray-300 text-gray-700 text-sm font-medium px-3 py-1.5 rounded-[4px]">← Retour</a>
            </div>
        </div>

        @if($rfq->awardedQuote)
        <div class="mt-4 bg-emerald-50 border border-emerald-200 rounded-[4px] p-3 text-sm text-emerald-800">
        ...
        <div class="bg-white rounded-[4px] border border-gray-300 overflow-hidden">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr>
                        <td class="px-3 py-1.5 text-sm">{{ $rs->supplier?->name ?? '—' }}</td>
                        ...
                        <td colspan="3" class="px-3 py-1.5">
                            <form action="{{ route('achats.rfq.record-quote', $rfq) }}" method="POST" class="space-y-3">
                                ...
                                <div class="flex justify-end gap-2">
                                    ...
                                    <button type="submit" class="bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-medium px-3 py-1.5 rounded">Enregistrer la cotation</button>
                                </div>
                            </form>
                        </td>
                    </tr>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="{{ $q->is_winner ? 'bg-emerald-50/50' : '' }}">
                    <td class="px-3 py-1.5 text-sm font-medium text-gray-900">
                    ...
                    <td class="px-3 py-1.5 text-right text-xs text-gray-600">{{ $q->valid_until?->format('d/m/Y') ?? '—' }}</td>
                    <td class="px-3 py-1.5 text-center">
                        @if($q->is_winner)
