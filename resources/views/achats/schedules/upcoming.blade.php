...
<div class="space-y-3">
    <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-3">
        <div>
            <h1 class="text-[16px] font-bold text-gray-900">💰 Échéances de paiement</h1>
        ...
        <form method="GET" class="flex items-end gap-2">
            <div>
                ...
                <select name="window" onchange="this.form.submit()" class="border border-gray-300 rounded-[4px] px-3 py-2 text-sm">
    ...
    <div class="grid grid-cols-2 gap-4">
        <div class="bg-white rounded-[4px] border-2 {{ count($overdue) > 0 ? 'border-red-300' : 'border-emerald-200' }} p-5">
            ...
            <p class="mt-1 text-[16px] font-bold tabular-nums {{ count($overdue) > 0 ? 'text-red-700' : 'text-emerald-700' }}">{{ count($overdue) }}</p>
        ...
        <div class="bg-white rounded-[4px] border border-amber-200 p-5">
            ...
            <p class="mt-1 text-[16px] font-bold tabular-nums text-amber-700">{{ count($upcoming) }}</p>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-red-50/30">
                    ...
                    <td class="px-3 py-1.5 text-xs">{{ $s->supplierInvoice?->supplier?->name }}</td>
                    <td class="px-3 py-1.5 text-xs">{{ $s->label ?? 'Éch. '.$s->installment_number }}</td>
                    ...
                    <td class="px-3 py-1.5 text-right text-xs text-red-700 font-medium">{{ $s->due_date?->format('d/m/Y') }} <span class="text-red-500">(+{{ $s->due_date?->diffInDays(now()) }} j)</span></td>
                </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-3 py-1.5 text-right text-xs text-gray-600">{{ $s->due_date?->format('d/m/Y') }} <span class="text-gray-400">({{ now()->diffInDays($s->due_date, false) }} j)</span></td>
                </tr>
                @endforeach
            </tbody>
        </table>
