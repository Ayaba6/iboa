...
@section('breadcrumb')
    ...
    <span class="mx-1">/</span>
    <a href="{{ route('crm.opportunities.show', $opportunity) }}" class="hover:text-gray-700">{{ Str::limit($opportunity->title, 30) }}</a>
    <span class="mx-1">/</span>
    <span class="text-gray-900 font-medium">Modifier</span>
@endsection

@section('content')
<form method="POST" action="{{ route('crm.opportunities.update', $opportunity) }}" class="space-y-3">
    @csrf
    @method('PUT')

    <div class="flex items-center justify-between">
        <h1 class="text-[16px] font-bold text-gray-900">Modifier opportunité</h1>
