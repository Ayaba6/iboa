...
<div class="space-y-3"
    ...
    <div class="bg-white rounded-[4px] border border-gray-300 p-4">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-3 flex-wrap">
                <h1 class="text-[20px] font-bold text-gray-900 leading-tight">Facture <span class="font-mono text-emerald-700 text-[20px]">{{ $invoice->number }}</span></h1>
                @php
                    $statusBadges = [
                ...
                @endif
                <span class="text-gray-500 text-[13px]">{{ $invoice->client?->name }}</span>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                @if($invoice->status === 'brouillon')
                    ...
                    <form action="{{ route('ventes.factures.submit', $invoice) }}" method="POST"
                        ...
                        @csrf
                        <button type="submit" class="inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-700 text-white rounded-[4px] text-[13px] font-semibold hover:bg-emerald-800 transition-colors shadow-sm">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @if($invoice->status === 'en_attente_validation')
                    ...
                    <form action="{{ route('ventes.factures.validate-internal', $invoice) }}" method="POST"
                        ...
                        @csrf
                        <button type="submit" class="inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-600 text-white rounded-[4px] text-[13px] font-semibold hover:bg-emerald-700 transition-colors shadow-sm">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <form action="{{ route('ventes.factures.reject-internal', $invoice) }}" method="POST"
                        ...
                        <button type="button" @click="open = true"
                                class="inline-flex items-center gap-2 px-3 py-2 border border-orange-200 text-orange-600 rounded-[4px] text-[13px] font-medium hover:bg-orange-50 transition-colors">
                            Refuser
                        </button>
                        <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50">
                            <div class="bg-white rounded-[4px] p-6 shadow-2xl w-full max-w-md mx-4">
                                ...
                                <textarea x-model="motif" rows="3" class="w-full border border-gray-300 rounded-[4px] px-3 py-2 text-[13px]" placeholder="Motif obligatoire (5 caractères min.)…"></textarea>
                                <div class="flex justify-end gap-2 mt-4">
                                    <button type="button" @click="open = false" class="btn btn-secondary">Annuler</button>
                                    <button type="submit" class="inline-flex items-center gap-2 px-3 py-1.5 bg-orange-600 text-white rounded-[4px] text-[13px] font-semibold hover:bg-orange-700 transition-colors">Confirmer le refus</button>
                                </div>
                            </div>
                        </div>
                    </form>
                    ...
                    <form action="{{ route('ventes.factures.cancel-internal', $invoice) }}" method="POST"
                        ...
                        <input type="hidden" name="motif" x-model="motif">
                        <button type="button" @click="open = true"
                                class="inline-flex items-center gap-2 px-3 py-2 border border-red-200 text-red-600 rounded-[4px] text-[13px] font-medium hover:bg-red-50 transition-colors">
                            Annuler
                        </button>
                        <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50">
                            <div class="bg-white rounded-[4px] p-6 shadow-2xl w-full max-w-md mx-4">
                                ...
                                <div class="flex justify-end gap-2 mt-4">
                                    ...
                                    <button type="submit" class="inline-flex items-center gap-2 px-3 py-1.5 bg-red-600 text-white rounded-[4px] text-[13px] font-semibold hover:bg-red-700 transition-colors">Confirmer l'annulation</button>
                                </div>
                            </div>
                        </div>
                    </form>
                ...
                <a href="{{ route('tresorerie.encaissements.create', ['client_id' => $invoice->client_id, 'invoice_id' => $invoice->id]) }}"
                   class="inline-flex items-center gap-2 px-3 py-2 bg-emerald-600 text-white rounded-[4px] text-[13px] font-medium hover:bg-emerald-700 transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button type="button" disabled
                        ...
                        aria-disabled="true"
                        class="inline-flex items-center gap-2 px-3 py-2 bg-gray-300 text-gray-500 rounded-[4px] text-[13px] font-medium cursor-not-allowed opacity-70 select-none">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button type="button" @click="showCancelModal = true; cancelReason = ''"
                        class="inline-flex items-center gap-2 px-3 py-2 border border-red-300 text-red-600 hover:bg-red-50 rounded-[4px] text-[13px] font-medium transition-colors"
    ...
    <nav class="flex items-stretch border-b border-gray-200 gap-1 bg-white rounded-t-[4px] px-2" x-data="{ atab: 'entete' }">
        ...
        <button type="button" @click="atab = '{{ $tk }}'; $refs['sec_{{ $tk }}']?.scrollIntoView({ behavior: 'smooth', block: 'start' })"
                class="px-3 py-2 text-[13px] font-semibold border-b-2 transition-colors whitespace-nowrap"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-3 items-start scroll-mt-24" x-ref="sec_entete">
        <div class="lg:col-span-2 bg-white rounded-[4px] border border-gray-200 p-4 space-y-4">
            ...
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-[13px]">
                <div>
                    ...
                    @if($invoice->client?->phone)<dd class="text-gray-500 text-[12px]">{{ $invoice->client->phone }}</dd>@endif
                    @if($invoice->client?->email)<dd class="text-gray-500 text-[12px]">{{ $invoice->client->email }}</dd>@endif
                    @php
                    ...
                    @endphp
                    @if($addr)
                    <dd class="text-gray-500 text-[12px] mt-1 leading-snug">
                ...
                <div>
                    <dt class="text-[12px] font-medium text-gray-500 uppercase tracking-wider">Date d'échéance</dt>
                ...
                <div>
                    <dt class="text-[12px] font-medium text-gray-500 uppercase tracking-wider">Conditions paiement</dt>
                    <dd class="mt-0.5 text-gray-700">{{ $invoice->payment_terms }}</dd>
                </div>
                @endif
                @if($invoice->order)
                <div>
                    <dt class="text-[12px] font-medium text-gray-500 uppercase tracking-wider">Commande d'origine</dt>
                    <dd class="mt-0.5">
                ...
                <div class="sm:col-span-2">
                    ...
                    <dd class="mt-0.5 text-gray-700 whitespace-pre-wrap text-[12px]">{{ $invoice->notes }}</dd>
                </div>
                @endif
            </dl>
        </div>
        ...
        <div class="bg-white rounded-[4px] border border-gray-200 p-4 space-y-3 h-fit">
            ...
            <div class="border-t border-gray-200 pt-3 flex justify-between">
                <span class="text-[13px] font-bold text-gray-900">Montant TTC</span>
                <span class="text-[13px] font-bold text-gray-900 tabular-nums">{{ number_format($invoice->total_ttc, 0, ',', ' ') }} FCFA</span>
            </div>

            {{-- Retenues à la source --}}
            @if(!empty($invoice->withholding_details))
                @foreach($invoice->withholding_details as $w)
                <div class="flex justify-between text-[13px] text-amber-700">
            ...
            <div class="border-t-2 border-emerald-200 pt-3 flex justify-between">
                <span class="text-[13px] font-bold text-gray-900">NET À PAYER</span>
                <span class="text-[13px] font-bold text-emerald-800 tabular-nums">{{ number_format($invoice->net_to_pay ?: $invoice->total_ttc, 0, ',', ' ') }} FCFA</span>
            </div>

            @if($invoice->paid_amount > 0)
            <div class="flex justify-between text-[13px] text-gray-600 border-t border-gray-100 pt-2">
                <span>Déjà payé</span>
                <span class="font-medium tabular-nums text-green-600">{{ number_format($invoice->paid_amount, 0, ',', ' ') }} FCFA</span>
            </div>
            @endif
            @if($invoice->remaining_amount > 0)
            <div class="flex justify-between text-[13px] border-t border-gray-100 pt-2">
            ...
            <table class="min-w-full text-[13px] border-collapse">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="odd:bg-white even:bg-gray-50/40 hover:bg-emerald-50/50 transition-colors">
                        <td class="px-3 py-1 text-gray-400 text-[12px]">{{ $loop->iteration }}</td>
            ...
            <table class="min-w-full text-[13px] border-collapse">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="odd:bg-white even:bg-gray-50/40 hover:bg-emerald-50/50 transition-colors">
                        ...
                        <td class="px-3 py-1 text-gray-400 text-[12px] hidden lg:table-cell">
                            {{ $pmt->reference ?: '—' }}
                        </td>
                        <td class="px-3 py-1 text-right">
                            <a href="{{ route('tresorerie.encaissements.show', $pmt) }}"
                               class="text-[12px] text-emerald-700 hover:text-emerald-900 font-medium">
                                Détail →
                            </a>
                        </td>
                    </tr>
    ...
    <div class="bg-white rounded-[4px] border border-gray-300 overflow-hidden">
        <div class="px-4 py-2 border-b border-gray-200 bg-[#eef5f0] flex items-center justify-between">
            ...
            <a href="{{ route('ventes.avoirs.create', ['invoice_id' => $invoice->id]) }}"
               class="text-[12px] text-emerald-700 hover:text-emerald-900 font-medium border border-emerald-200 hover:bg-emerald-50 px-2.5 py-1 rounded-[4px] transition-colors">
                + Nouvel avoir
            </a>
        </div>
        <table class="min-w-full text-[13px] border-collapse">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="odd:bg-white even:bg-gray-50/40 hover:bg-emerald-50/50 transition-colors">
                    ...
                    <td class="px-3 py-1 text-gray-500 text-[12px] hidden md:table-cell">{{ $cn->reason ?? '—' }}</td>
...
{{-- [UI — doublon retiré] La section « 7. Historique » affichait le MÊME journal         d'audit que le composant <x-audit.timeline> appelé plus bas : deux panneaux         identiques sur une seule page, et deux fois la même requête SQL.         Le composant est conservé — il sert ONZE fiches (devis, commandes, avoirs,         BL, achats, réceptions, transferts, journaux) et affiche davantage :         adresse IP, écart avant/après, référence du document. Les quatre actions         qu'il ne nommait pas encore — sent, cancelled, partially_paid, overdue —         y ont été ajoutées avant cette suppression, pour ne rien perdre. --}}
    ...
    <div x-show="showCancelModal" x-cloak
        ...
        <div class="bg-white rounded-[4px] shadow-2xl w-full max-w-md" @click.outside="showCancelModal = false">
            <div class="px-3 py-1.5 border-b border-gray-200">
                <h3 class="text-[13px] font-semibold text-gray-900 flex items-center gap-2">
            ...
            <form action="{{ route('ventes.factures.cancel', $invoice) }}" method="POST" data-turbo="false">
                ...
                <div class="px-6 py-5 space-y-3">
                    <div class="bg-amber-50 border border-amber-200 rounded-[4px] p-3 text-[12px] text-amber-800">
                    ...
                    <div>
                        ...
                        <textarea name="reason" x-model="cancelReason" rows="3" required minlength="5" maxlength="500"
                                  ...
                                  class="w-full border border-gray-300 rounded-[4px] px-3 py-2 text-[13px] focus:ring-1 focus:ring-red-500 focus:border-red-500 resize-none"></textarea>
                        <p class="text-[11px] text-gray-400 mt-1">Conservé dans l'historique d'audit comptable</p>
                    </div>
                </div>
                <div class="px-6 py-3 bg-gray-50 border-t border-gray-200 flex justify-end gap-2">
                    <button type="button" @click="showCancelModal = false"
                            class="border border-gray-300 text-gray-700 text-[13px] font-medium px-3 py-1.5 rounded-[4px] hover:bg-white">
                        Fermer
                    </button>
                    <button type="submit" :disabled="!canSubmitCancel"
                            class="bg-red-600 hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed text-white text-[13px] font-medium px-3 py-1.5 rounded-[4px] transition-colors">
    ...
    <div class="bg-white rounded-[4px] border border-emerald-200 overflow-hidden scroll-mt-24" x-ref="sec_echeancier"
        ...
        <div class="px-4 py-2 bg-[#eef5f0] border-b border-emerald-200 flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                @if($schedules->count())
                <span class="text-[12px] bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded-full font-medium">
            ...
            <div class="flex items-center gap-2">
                ...
                <button @click="tab = (tab === 'view' ? 'create' : 'view')"
                        class="text-[12px] text-emerald-700 hover:text-emerald-900 font-medium underline">
        ...
        <div x-show="tab === 'view'">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="odd:bg-white even:bg-gray-50/40 hover:bg-emerald-50/50 transition-colors {{ $isLate ? '!bg-rose-50' : '' }}">
                        ...
                        <td class="px-3 py-1 text-center tabular-nums {{ $isLate ? 'text-rose-700 font-semibold' : 'text-gray-700' }}">
                            ...
                            @if($isLate)
                            <span class="ml-1 text-[12px] text-rose-600 font-bold">
                        ...
                        <td class="px-3 py-1 text-center">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-[3px] text-[11px] font-medium {{ $schBadges[$sch->status] ?? 'bg-gray-100 text-gray-700' }}">
            ...
            <div class="px-3 py-1.5 border-t border-emerald-100 flex justify-end">
                <form action="{{ route('ventes.factures.schedules.destroy-all', $invoice) }}" method="POST"
                    ...
                    @csrf @method('DELETE')
                    <button type="submit"
                            class="text-[12px] text-red-500 hover:text-red-700 font-medium flex items-center gap-1">
                        ...
                        <template x-for="(row, i) in rows" :key="i">
                            <div class="flex items-center gap-2">
                                <input type="number" :name="'installments['+i+'][percent]'"
                                       ...
                                       class="w-20 border border-gray-300 rounded-[4px] px-2.5 py-1.5 text-[13px] text-center focus:ring-1 focus:ring-emerald-400">
                                ...
                                <input type="number" :name="'installments['+i+'][days_after]'"
                                       ...
                                       class="w-28 border border-gray-300 rounded-[4px] px-2.5 py-1.5 text-[13px] focus:ring-1 focus:ring-emerald-400">
                                <span class="text-gray-400 text-[12px] whitespace-nowrap">j. après émission</span>
                                <input type="text" :name="'installments['+i+'][label]'"
                                       x-model="row.label" placeholder="Libellé (optionnel)"
                                       class="flex-1 border border-gray-300 rounded-[4px] px-2.5 py-1.5 text-[13px] focus:ring-1 focus:ring-emerald-400">
                    ...
                    <div class="flex items-center justify-between pt-1">
                        <div class="flex items-center gap-3">
                            <button type="button" @click="rows.push({ percent: 0, days_after: 30, label: '' })"
                                    class="text-[13px] text-emerald-700 hover:text-emerald-900 font-medium flex items-center gap-1">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <button type="submit"
                                class="inline-flex items-center gap-2 bg-emerald-700 hover:bg-emerald-800 text-white text-[13px] font-medium px-3 py-1.5 rounded-[4px] transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <template x-for="(row, i) in customRows" :key="i">
                            <div class="flex items-center gap-2">
                                <input type="date" :name="'rows['+i+'][due_date]'"
                                       ...
                                       class="border border-gray-300 rounded-[4px] px-2.5 py-1.5 text-[13px] focus:ring-1 focus:ring-emerald-400">
                                <input type="number" :name="'rows['+i+'][amount]'"
                                       x-model="row.amount" min="1" required
                                       placeholder="Montant FCFA"
                                       class="w-40 border border-gray-300 rounded-[4px] px-2.5 py-1.5 text-[13px] tabular-nums focus:ring-1 focus:ring-emerald-400">
