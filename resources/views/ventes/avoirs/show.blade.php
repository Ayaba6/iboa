...
<div class="space-y-3">
    ...
    <div class="bg-white rounded-[4px] border border-gray-300 p-5">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-3 flex-wrap">
                <h1 class="text-[15px] font-bold text-gray-900 font-mono">{{ $creditNote->number }}</h1>
                @php
                    $badges = [
                ...
                @endphp
                <span class="inline-flex items-center px-2.5 py-1 rounded-[3px] text-[11px] font-semibold {{ $badges[$creditNote->status] ?? 'bg-gray-100 text-gray-700' }}">
                    {{ $labels[$creditNote->status] ?? $creditNote->status }}
                </span>
                <span class="text-gray-500 text-[13px]">{{ $creditNote->client?->name }}</span>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                @php
                    $avBtnO  = 'inline-flex items-center gap-2 px-3 py-2 border border-gray-300 text-gray-700 rounded-[4px] text-[13px] font-medium hover:bg-gray-50 transition-colors';
                    $avBtnP  = 'inline-flex items-center gap-2 px-3 py-1.5 text-white rounded-[4px] text-[13px] font-semibold shadow-sm transition-colors';
                    $avBtnWO = 'inline-flex items-center gap-2 px-3 py-2 border border-orange-200 text-orange-600 rounded-[4px] text-[13px] font-medium hover:bg-orange-50 transition-colors';
                    $avBtnDO = 'inline-flex items-center gap-2 px-3 py-2 border border-red-200 text-red-600 rounded-[4px] text-[13px] font-medium hover:bg-red-50 transition-colors';
                @endphp
                        ...
                        <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50">
                            <div class="bg-white rounded-[4px] p-6 shadow-2xl w-full max-w-md mx-4">
                                ...
                                <div class="flex justify-end gap-2 mt-4">
                                    ...
                                    <button type="submit" class="{{ $avBtnP }} bg-emerald-700 hover:bg-emerald-800">Confirmer le refus</button>
                                </div>
                            </div>
                        </div>
                    </form>
                ...
                <form action="{{ route('ventes.avoirs.replacement', $creditNote) }}" method="POST"
                    ...
                    @csrf
                    <button type="submit"
                            class="inline-flex items-center gap-2 px-3 py-2 bg-amber-600 text-white rounded-[4px] text-[13px] font-medium hover:bg-amber-700 transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 bg-white rounded-[4px] border border-gray-300 p-5 space-y-4">
            ...
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-[13px]">
                <div>
                    ...
                    @if($creditNote->client?->phone)<dd class="text-gray-500 text-[12px]">{{ $creditNote->client->phone }}</dd>@endif
                </div>
                <div>
                    <dt class="text-[12px] font-medium text-gray-500 uppercase tracking-wider">Numéro avoir</dt>
                ...
                <div>
                    <dt class="text-[12px] font-medium text-gray-500 uppercase tracking-wider">Facture liée</dt>
                    <dd class="mt-0.5">
                        <a href="{{ route('ventes.factures.show', $creditNote->invoice) }}" class="font-mono font-semibold text-emerald-700 hover:text-emerald-900">
                ...
                <div class="sm:col-span-2">
                    <dt class="text-[12px] font-medium text-gray-500 uppercase tracking-wider">Motif</dt>
                ...
                <div class="sm:col-span-2">
                    ...
                    <dd class="mt-0.5 text-gray-700 whitespace-pre-wrap text-[12px]">{{ $creditNote->notes }}</dd>
                </div>
                @endif
                @if($creditNote->validatedBy)
                <div>
                    <dt class="text-[12px] font-medium text-gray-500 uppercase tracking-wider">Validé par</dt>
        ...
        <div class="bg-white rounded-[4px] border border-gray-300 p-5 space-y-3 h-fit">
            ...
            <div class="border-t border-gray-200 pt-3 flex justify-between">
                ...
                <span class="text-[13px] font-bold text-purple-700 tabular-nums">{{ number_format($creditNote->total_ttc, 0, ',', ' ') }} FCFA</span>
            </div>
    ...
    <div class="bg-white rounded-[4px] border border-gray-300 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-200">
            <h2 class="text-[13px] font-semibold text-gray-900">Lignes de l'avoir</h2>
        </div>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-3 py-1.5">
                            @if(($item->disposition ?? 'restock') === 'rebut')
                                <span class="inline-flex items-center px-2 py-0.5 rounded-[3px] text-[11px] font-semibold bg-red-100 text-red-700">Rebut</span>
                            @else
                                <span class="inline-flex items-center px-2 py-0.5 rounded-[3px] text-[11px] font-semibold bg-emerald-100 text-emerald-700">Remis en stock</span>
                            @endif
                        </td>
