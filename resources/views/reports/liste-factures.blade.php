...
@section('breadcrumb')
    ...
    <span class="mx-1">/</span>
    <span class="text-gray-900 font-medium">Liste des factures</span>
@endsection
...
<div class="space-y-3">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            <h1 class="text-[16px] font-bold text-gray-900">Liste des factures</h1>
            <p class="text-sm text-gray-500 mt-0.5">Toutes les factures avec filtres avancés — FCFA</p>
        </div>
    ...
    <form method="GET" class="bg-white rounded-[4px] border border-gray-300 p-4">
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div>
                ...
                <select name="status" class="w-full border border-gray-300 rounded-[4px] px-3 py-2 text-sm bg-white focus:ring-1 focus:ring-emerald-400">
        ...
        <div class="mt-3 flex gap-2">
            ...
            <a href="{{ route('reports.liste-factures') }}" class="border border-gray-300 text-gray-600 hover:bg-gray-50 text-sm px-2.5 py-1.5 rounded-[4px]">Réinitialiser</a>
        </div>
    </form>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div class="bg-white rounded-[4px] border border-gray-300 p-4">
            ...
            <p class="mt-1 text-[17px] font-bold text-emerald-800">{{ $rows->count() }}</p>
        </div>
        <div class="bg-white rounded-[4px] border border-gray-300 p-4">
        ...
        <table class="w-full text-sm">
            <thead class="bg-emerald-700 text-white">
                <tr>
                    ...
                    <th class="px-3 py-1.5 text-right font-semibold">Restant</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
                ...
                @php
                    $sc = match($r->status) {
                        'payee' => 'green', 'partiellement_payee' => 'amber',
                        'en_retard' => 'red', 'annulee' => 'red',
                        'envoyee' => 'blue', 'validee' => 'blue', default => 'gray',
                    };
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-3 py-2.5 text-center text-gray-600">{{ $r->due_at?->format('d/m/Y') ?? '—' }}</td>
                    <td class="px-3 py-2.5 text-gray-800">{{ $r->client?->name ?? '—' }}</td>
                    <td class="px-3 py-2.5 text-center">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-[3px] text-[11px] font-medium bg-{{ $sc }}-100 text-{{ $sc }}-800">{{ $sl }}</span>
                    </td>
                    <td class="px-3 py-2.5 text-right tabular-nums text-gray-700">{{ number_format($r->subtotal_ht, 0, ',', ' ') }}</td>
                    <td class="px-3 py-2.5 text-right tabular-nums font-medium text-gray-900">{{ number_format($r->total_ttc, 0, ',', ' ') }}</td>
                    <td class="px-3 py-2.5 text-right tabular-nums text-emerald-700">{{ number_format($r->paid_amount, 0, ',', ' ') }}</td>
                    <td class="px-3 py-2.5 text-right tabular-nums font-semibold {{ $r->remaining_amount > 0 ? 'text-rose-700' : 'text-gray-400' }}">
                        {{ number_format($r->remaining_amount, 0, ',', ' ') }}
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="9" class="px-4 py-12 text-center text-gray-400">Aucune facture sur cette période</td>
                </tr>
                @endforelse
            </tbody>
            @if($rows->count())
            <tfoot class="bg-indigo-900 text-white font-bold">
                <tr>
                    <td class="px-3 py-1.5" colspan="5">TOTAL ({{ $rows->count() }} facture{{ $rows->count() > 1 ? 's' : '' }})</td>
