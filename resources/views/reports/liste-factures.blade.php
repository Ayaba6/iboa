...
<div class="space-y-3">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div class="bg-white rounded-[4px] border border-gray-300 px-3 py-2">
            <p class="text-[10px] font-bold text-gray-500 uppercase tracking-wide">Nb factures</p>
        ...
        <div class="bg-white rounded-[4px] border border-gray-300 px-3 py-2">
            <p class="text-[10px] font-bold text-gray-500 uppercase tracking-wide">Restant dû</p>
        ...
        <table class="w-full text-[14px] border-collapse">
            <thead class="bg-[#3b4248] text-[12px] font-semibold text-white">
                <tr>
                    ...
                    <th class="px-3 py-1.5 text-right">Restant</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
                ...
                @php
                    $sc = match($r->status) {
                        'payee' => 'bg-green-100 text-green-800', 'partiellement_payee' => 'bg-amber-100 text-amber-800',
                        'en_retard' => 'bg-red-100 text-red-800', 'annulee' => 'bg-red-100 text-red-800',
                        'envoyee' => 'bg-blue-100 text-blue-800', 'validee' => 'bg-blue-100 text-blue-800', default => 'bg-gray-100 text-gray-800',
                    };
                ...
                <tr class="odd:bg-white even:bg-gray-50/40 hover:bg-emerald-50/50 transition-colors">
                    ...
                    <td class="px-3 py-1 text-center text-gray-600">{{ $r->due_at?->format('d/m/Y') ?? '—' }}</td>
                    ...
                    <td class="px-3 py-1 text-right tabular-nums font-medium text-gray-900">{{ number_format($r->total_ttc, 0, ',', ' ') }}</td>
                    <td class="px-3 py-1 text-right tabular-nums text-emerald-700">{{ number_format($r->paid_amount, 0, ',', ' ') }}</td>
                    <td class="px-3 py-1 text-right tabular-nums font-semibold {{ $r->remaining_amount > 0 ? 'text-rose-700' : 'text-gray-400' }}">
