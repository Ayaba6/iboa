...
@section('content')
@php
    $card = 'bg-white border border-gray-300 rounded-[4px] overflow-hidden';
    $secH = 'px-3 py-1.5 border-b border-gray-200 bg-[#eef5f0]';
    $lbl  = 'block text-[11px] font-semibold text-gray-600 mb-1';
    $inp  = 'w-full h-9 border border-gray-300 rounded-[4px] px-3 text-[13px] bg-white focus:outline-none focus:ring-1 focus:ring-emerald-400 focus:border-emerald-500';
    $btn  = 'text-[12.5px] font-semibold px-4 py-1.5 rounded-[4px] transition-colors';
    $err  = 'mt-1 text-[11px] text-red-600';
@endphp
...
<div class="space-y-3">
    ...
    {{-- Bandeau titre --}}
    <div>
        <h1 class="text-[16px] font-bold text-gray-900">Mon profil</h1>
        <p class="text-[11.5px] text-gray-400">Gérez vos informations personnelles et la sécurité de votre compte.</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-3 items-start">
        ...
        <div class="lg:col-span-2 space-y-3">
            ...
            <div class="{{ $card }}">
                <div class="{{ $secH }}"><h2 class="text-[13px] font-bold text-emerald-900">Informations du profil</h2></div>
                ...
                <form method="POST" action="{{ route('profile.update') }}" class="p-4 space-y-3">
                    ...
                    @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                        <div class="p-2.5 bg-amber-50 border border-amber-200 rounded-[4px]">
                            <p class="text-[11.5px] text-amber-700">Votre adresse e-mail n'est pas vérifiée.
                                <button form="send-verification" class="underline font-semibold hover:text-amber-900 ml-1">Renvoyer l'e-mail</button>
                            </p>
                            @if (session('status') === 'verification-link-sent')
                                <p class="mt-1 text-[11.5px] font-semibold text-green-600">Un nouveau lien de vérification a été envoyé.</p>
                            @endif
                        </div>
                    @endif

                    {{-- [CDC §Workflow] Canal e-mail pour les notifications de validation --}}
                    <label class="flex items-start gap-2.5 cursor-pointer rounded-[4px] border border-gray-200 p-2.5">
                        ...
                        <span>
                            <span class="block text-[12.5px] font-semibold text-gray-900">Notifications de validation par e-mail</span>
                            <span class="block text-[11px] text-gray-500 mt-0.5">Les notifications internes (cloche) restent toujours actives. Cette option envoie en plus un e-mail pour chaque document soumis à votre validation.</span>
                        </span>
                    </label>
            ...
            <div id="password" class="{{ $card }} scroll-mt-20">
                <div class="{{ $secH }}"><h2 class="text-[13px] font-bold text-emerald-900">Modifier le mot de passe</h2></div>
        ...
        <div class="space-y-3">
            <div class="{{ $card }}">
                <div class="{{ $secH }}"><h2 class="text-[13px] font-bold text-emerald-900">Compte</h2></div>
                <div class="p-4 flex items-center gap-3">
                    ...
                    <div class="min-w-0">
                        <p class="text-[13px] font-bold text-gray-900 truncate">{{ $user->name }}</p>
                        <p class="text-[11px] text-gray-400 truncate">{{ $user->getRoleNames()->first() ?? 'Utilisateur' }}</p>
                    </div>
                </div>
            </div>

            <div class="{{ $card }} !border-red-200" x-data="{ open: false }">
                <div class="px-3 py-1.5 border-b border-red-200 bg-red-50"><h2 class="text-[13px] font-bold text-red-700">Zone de danger</h2></div>
                <div class="p-4">
                    <p class="text-[11.5px] text-gray-500 mb-3">Une fois votre compte supprimé, toutes ses données seront définitivement effacées.</p>
                    ...
                    <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
                        ...
                        <div class="relative bg-white rounded-[4px] shadow-xl w-full max-w-md p-5 z-10"
                            ...
                            <div class="flex items-start gap-3 mb-3">
                                ...
                                <div>
                                    <h3 class="text-[14px] font-bold text-gray-900">Confirmer la suppression</h3>
                                    <p class="text-[12px] text-gray-500 mt-0.5">Action irréversible. Entrez votre mot de passe pour confirmer.</p>
                                </div>
                            </div>
                            <form method="POST" action="{{ route('profile.destroy') }}" class="space-y-3">
                                ...
                                <div>
                                    ...
                                    <input id="del_password" name="password" type="password" placeholder="Votre mot de passe actuel"
                                           class="w-full h-9 border border-gray-300 rounded-[4px] px-3 text-[13px] focus:outline-none focus:ring-1 focus:ring-red-400 focus:border-red-500">
                                    @error('password', 'userDeletion')<p class="{{ $err }}">{{ $message }}</p>@enderror
                                </div>
                                <div class="flex justify-end gap-2 pt-0.5">
                                    <button type="button" @click="open = false" class="{{ $btn }} border border-gray-300 text-gray-700 hover:bg-gray-50">Annuler</button>
