...
| **Décaissement fournisseur** | allocations FF, GL, caisse | `SupplierPaymentService::cancel` (existant) | factures restaurées, extourne, caisse re-créditée, statut+motif | **PARTIEL** (mécanisme complet, test dédié absent) | test miroir à écrire |
...
| **Réception fournisseur** | stock, bobines/lots, PO reçu | **AUCUN CANCEL** | — | **MANQUANT** | à implémenter : annulation technique (non facturée + bobines non consommées) = contre-mouvements + suppression bobines/lots + PO re-ouvert ; sinon renvoi = retour fournisseur ; réception facturée = inannulable |
